...
def parse_args():
    ...
    parser.add_argument('--sampler_method',type=str,default=None,help='数据采样方法')
    parser.add_argument('--encoder_method',type=str,default=None,help='编码器方法')
    parser.add_argument('--cluster_method',type=str,default=None,help='聚类方法')
...
if __name__ == "__main__":
    ...
    if args.sampler_method == 'aug':
        embedding_tensor = torch.from_numpy(np.load(args.EmbeddingPath + args.embedding_tensor_name + '_aug' + '.npy'))
    elif args.encoder_method == 'enc':
        embedding_tensor = torch.from_numpy(np.load(args.EmbeddingPath + args.embedding_tensor_name + '_enc' + '.npy'))
    elif args.cluster_method == 'kmedoids':
        embedding_tensor = torch.from_numpy(np.load(args.EmbeddingPath + args.embedding_tensor_name + '_clu' + '.npy'))
    else:
    ...
    f1_score = f1_score_calculation(y_pred.int(), labels.int(),args)
    print("F1 score by maximum weight gain: {}".format(f1_score))
